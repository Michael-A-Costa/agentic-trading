...
The validated edge is: an overnight gap (vs. prior close) ≥ ~7% on ≥ 2× average daily volume, on a large-cap name, produces measurable forward drift over 10–20 trading days. Per-trade expectancy is positive (+1.4–1.95%, net of costs), but the edge is noisy (win rate ~45–49% unfiltered).

**Honesty caveat:** "large-cap" in the backtest was a fixed 60-name mega-cap list (S&P-100-class, all roughly ≥ $40B — `backtest_gap_drift.py`), not a market-cap threshold. The edge is validated **only** in that regime. The engine's discovery floor is $300M; everything below mega-cap trades as unproven discretion, not as PEAD. The two-book split below makes this distinction operational.

---

## 2b. Two Virtual Books (v2, 2026-06-09)

One account, two explicitly tagged books sharing one capital pool (`strategies/two-book-v2-plan.md`):

| | `pead` | `disco` |
|---|---|---|
| **What** | The measured edge: `pead_qualified` gap+vol signal on a mega-cap (≥ $30B proxy) | Free-rein agent discretion (everything else) |
| **Capital** | Ceiling: up to 30% of equity (`BOOK_PEAD_MAX_FRAC`); reserves **nothing** while idle | Everything pead isn't using, to the global 90% cap |
| **Priority** | First claim on settled cash within a tick | — |
| **Verdict** | ≥30 resolved events, no lift over gap-alone → disarm | ≥30 round-trips, negative net expectancy (or P&L tripwire ×2) → disarm |

Every DD verdict, lot, and trade row carries a `book` tag; reports split by it (`pnl_report.py --by-book`, `trade_ledger.py --book`, `catalyst_filter_report.py` book rows). Labels never spill — an unfunded pead commit is skipped and logged (`pead_unfunded`), never re-tagged. Disarming one book halts only its new entries; exits, stops, and the other book keep running. `BOOKS_ENABLED=0` runs tagging-only (Phase 0); `=1` enforces the ceiling, priority, ticker ownership, and per-book P&L tripwires (pead ≈ −$62, disco ≈ −$145 at the $2,064 baseline).

**Economics, stated plainly:** at ~$2k equity the expected gross edge is ~$4–6 per full-size trade, against real LLM cost per DD. At this size the account is an R&D vehicle — the deliverable is the forward ledger, not the P&L. The cost ledger (`data/costs.jsonl`, surfaced as the *edge vs spend* block in `pnl_report.py`) keeps that honest weekly.
...
**PEAD calendar** (`discover_pead.py`): pulls the Nasdaq earnings calendar for the last 4 calendar days. Any stock that reported earnings and passes basic eligibility (market cap ≥ $300M, not an ETF/fund, not on the exclusion list) is surfaced as a priority candidate. Fresh (day-0/1) reporters whose gap is **negative** are dropped before DD (`PEAD_DIRECTION=up`) — a long-only account can never trade them, so they no longer burn DD slots.
...
3. **Soft-cut (Tier-1 protective sell):** sell if a held position falls ≥ 8% intraday while below entry (`SOFT_CUT_PCT=8.0`). 8% is the backtested optimum — a 4% soft-cut was strictly worse than no soft-cut at all (it cut early into drift names, destroying ~0.4%/trade).
...
| Exit policy (8% soft-cut, no trailing stop until +20%) | Backtested on existing fills (2026-06-09) — a **tiny, single-regime sample**. Treat as a reasonable prior, not a validated optimum; re-run `backtest_exit_policy.py` once the ledger holds ≥30 round-trips (~the 2026-06-26 checkpoint). |
